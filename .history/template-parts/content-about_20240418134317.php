<div class="container">
    ...
    if ($founder_query->have_posts()) {
        while ($founder_query->have_posts()) {
            ...
            $founder_image = get_field('founder_1_image');
            $founder_title = get_field('founder_1_title');
            $founder_role = get_field('founde_1r_role');
            if ($founder_image) {
                echo wp_get_attachment_image($founder_image, 'full');
            }
            if ($founder_title) {
                echo '<h4 class="">' . $founder_title . '</h4>';
            }
            if ($founder_role) {
                echo '<h4 class="">' . $founder_role . '</h4>';
            }
        }
        wp_reset_postdata();
    }
